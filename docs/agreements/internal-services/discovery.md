# Discovery working group agreement

Discovery helps triage and vet challenges and opportunities and assists with the preparation and launch of action research projects and programming at EAR.

Discovery related resources (TK):

- Discovery session agenda 
- Discussion summary template: One or more related opportunities
- Opportunity summary (or...)
- Research plan template 

[#Ops_discovery](https://ecosystemaction.zulipchat.com/#narrow/channel/606970-Ops-Launch-Service) in Zulip

---

Exploration is subject to the [operations agreement](index.md).